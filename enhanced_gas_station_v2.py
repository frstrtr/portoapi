...
try:
    ...
    print(f"⚠️  TronPy not available. Install with: pip install tronpy\nError: {e}")
...
class ResourcePoolGasStation:
    ...
    def __init__(self, network: str = "nile"):
        ...
        print("🏭 Resource Pool Gas Station initialized")
        ...
        try:
            ...
            for freeze in frozen_v2:
                ...
                if freeze_type == "ENERGY":
                    ...
                    freeze_type == "NO_TYPE" or freeze_type is None or freeze_type == ""
                ):
            ...
            total_delegatable_energy = (
                frozen_energy  # Only energy stakes can delegate energy
            )
            total_delegatable_bandwidth = (
                frozen_bandwidth + frozen_other
            )  # Bandwidth + unrecognized
            ...
            # Total amounts
            total_frozen = (
                frozen_energy + frozen_bandwidth + frozen_other + frozen_tron_power
            )
            ...
            return {
                ...
                "bandwidth": {
                    ...
                    "frozen_declared": frozen_bandwidth
                    + frozen_other,  # All bandwidth-compatible stakes
                ...
                "debug_info": {
                    ...
                    "delegation_utilization": (
                        (total_effective / total_delegatable * 100)
        ...
        try:
            ...
            txn = (
                self.tron.trx.transfer(
                    from_=self.address, to=user_address, amount=activation_sun
                )
    ...
    def display_status(self):
        ...
        print(
            f"   Bottleneck: {min(energy_capacity, bandwidth_capacity, activation_capacity):,}"
        )
        ...
        print(f"\n📋 STAKING BREAKDOWN:")
        print(
            f"   Energy stakes: {debug['frozen_energy']:,.1f} TRX → Can only delegate ENERGY"
        )
        print(
            f"   Bandwidth stakes: {debug['frozen_bandwidth']:,.1f} TRX → Can only delegate BANDWIDTH"
        )
        ...
        unused_bandwidth = (
            bandwidth["delegatable_pool"] - debug["effective_bandwidth_stake"]
        )
        ...
        if unused_energy > 100 or unused_bandwidth > 100:
            ...
            print(
                f"   Unused BANDWIDTH delegation capacity: {unused_bandwidth:,.0f} TRX"
            )

            # Calculate what each pool could provide
            potential_energy_users = int(
                unused_energy * 32_000 / 65_000
            )  # 32k stake, 65k energy per tx
            potential_bandwidth_users = int(
                unused_bandwidth * 1_000 / 345
            )  # 1k stake, 345 bandwidth per tx

            print(f"   Could support {potential_energy_users:,} more ENERGY users")
            print(
                f"   Could support {potential_bandwidth_users:,} more BANDWIDTH users"
            )
        ...
        total_controlled = (
            status["balance"]
            + debug["total_delegatable_pool"]
            + debug["frozen_tron_power"]
        )
        ...
        if debug["total_delegatable_pool"] > 1000:
            ...
            print(
                f"   Energy pool: {energy['delegatable_pool']:,.0f} TRX for energy delegation"
            )
            print(
                f"   Bandwidth pool: {bandwidth['delegatable_pool']:,.0f} TRX for bandwidth delegation"
            )
            print(f"   This gives you enterprise-level gas station capabilities.")
            print(
                f"   Remember: Each pool can only delegate to its matching resource type!"
            )
