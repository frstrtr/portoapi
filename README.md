...
## Контрольный ключ для делегаций (без доступа к TRX)

Для повышения безопасности вы можете использовать отдельный «контрольный» ключ, который имеет права только на делегирование ресурсов (Freeze/Delegate), но не может переводить TRX. В этом режиме:

- Транзакции активации (перевод TRX) подписываются основным ключом GAS_WALLET_PRIVATE_KEY/…
- Делегирование ENERGY/BANDWIDTH подписывается контрольным ключом с указанным `permission_id` (TRON Account Permissions: Active Permission).

Переменные окружения:

```env
# Контрольный ключ (выберите один из способов):
GAS_WALLET_CONTROL_PRIVATE_KEY=hex-privkey
# или mnemonic+path (BIP44 TRON):
GAS_WALLET_CONTROL_MNEMONIC="seed words..."
GAS_WALLET_CONTROL_PATH="m/44'/195'/1'/0/0"

# Идентификатор Active Permission, к которому привязан контрольный ключ (обычно 2):
GAS_WALLET_CONTROL_PERMISSION_ID=2

# Разрешить откат к основному ключу для делегаций, если контрольный недоступен
# (в проде лучше false, чтобы строго гарантировать разделение полномочий)
GAS_CONTROL_FALLBACK_TO_OWNER=true
```

Шаги на стороне TRON-аккаунта:

1) Настройте Account Permissions в кошельке/через API: создайте Active Permission с нужными ограничениями и добавьте в него публичный ключ контрольного кошелька.
2) Ограничьте разрешённые операций/контрактов (freeze_balance с полем receiver) и, при необходимости, лимитируйте дневной лимит.
3) Узнайте `permission_id` созданного разрешения и укажите его в `.env`.

Поведение:

- Если контрольный ключ указан, делегирование ресурсов будет подписываться им (с `permission_id`).
- При `GAS_CONTROL_FALLBACK_TO_OWNER=false` делегирование не будет выполнено без контрольного ключа или при ошибке прав.
- При `GAS_CONTROL_FALLBACK_TO_OWNER=true` система попробует подписать делегацию основным ключом (менее безопасно, но повышает устойчивость).
