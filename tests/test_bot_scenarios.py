...
from bot.handlers import seller_handlers
import core.database.db_service as db_service
...
async def test_buyers_list():
    ...
    # Мокаем get_buyer_groups_by_seller из db_service
    db_service.get_buyer_groups_by_seller = MagicMock(
        return_value=[MagicMock(buyer_id="client1", invoices_group=0)]
    )
...
async def test_create_invoice_group_choice():
    ...
    seller_handlers.get_wallet_by_group = MagicMock(
        return_value=MagicMock(xpub="xpub_test")
    )
    seller_handlers.get_buyer_group = MagicMock(return_value=MagicMock(id=1))
    seller_handlers.create_invoice = MagicMock(
        return_value=MagicMock(address="T...", amount=10)
    )
