...
class GasStationService:
    ...
    def __init__(self, config=None):
        ...
        tron_network = 'nile' if self.network == 'testnet' else 'mainnet'
    ...
    def _get_usdt_contract(self) -> str:
        ...
        if self.network == 'testnet':
            return os.getenv('TRON_TESTNET_USDT_CONTRACT', 'TXYZopYRdj2D9XRtbG411XZZ3kM5VkAeBf')
        else:
            return os.getenv('TRON_MAINNET_USDT_CONTRACT', 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t')
    ...
    def activate_account(self, address: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if resources['balance'] < self.activation_cost:
                error_msg = f"Insufficient balance for activation: {resources['balance']} TRX"
    ...
    def delegate_resources_to_user(self, address: str, duration_days: int = 3) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Energy delegated: {energy_needed:,} to {address}, TX: {energy_txid}")
            ...
            try:
                ...
                logger.info(f"Bandwidth delegated: {bandwidth_needed:,} to {address}, TX: {bandwidth_txid}")
    ...
    def setup_seller_account(self, seller_address: str) -> Tuple[bool, Dict[str, str]]:
        ...
        try:
            ...
            success, delegation_txid = self.delegate_resources_to_user(seller_address, duration_days=7)
            if not success:
    ...
    def get_daily_capacity(self) -> Dict:
        ...
        daily_bandwidth = resources['bandwidth']['limit'] + resources['bandwidth']['free_limit']

        # Transaction capacity
        energy_tx_capacity = daily_energy // self.energy_per_usdt_tx if daily_energy > 0 else 0
        bandwidth_tx_capacity = daily_bandwidth // self.bandwidth_per_tx if daily_bandwidth > 0 else 0
        activation_capacity = int(resources['balance'] / self.activation_cost) if self.activation_cost > 0 else 0
    ...
    def get_status(self) -> Dict:
        ...
        if resources['energy']['staked'] > 0:
            energy_efficiency = (resources['energy']['limit'] / (resources['energy']['staked'] * 32_000)) * 100
        
        if resources['bandwidth']['staked'] > 0:
            bandwidth_efficiency = (resources['bandwidth']['limit'] / (resources['bandwidth']['staked'] * 1_000)) * 100
    ...
    def auto_rebalance(self) -> bool:
        ...
        try:
            ...
            energy_usage = (resources['energy']['used'] / max(resources['energy']['limit'], 1)) * 100
            bandwidth_usage = (resources['bandwidth']['used'] / max(resources['bandwidth']['limit'], 1)) * 100

            logger.info(f"Resource usage - Energy: {energy_usage:.1f}%, Bandwidth: {bandwidth_usage:.1f}%")
            ...
            if energy_usage > 80 and resources['balance'] > 100:
                ...
                try:
                    ...
                    result = txn.broadcast()
                    logger.info(f"Auto-staked {stake_amount} TRX for energy: {result.get('txid')}")
                    rebalanced = True
            ...
            if bandwidth_usage > 80 and resources['balance'] > 50:
                ...
                try:
                    ...
                    result = txn.broadcast()
                    logger.info(f"Auto-staked {stake_amount} TRX for bandwidth: {result.get('txid')}")
                    rebalanced = True
